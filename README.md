...
| -------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `LISTEN`                                                       | required — `0.0.0.0:8080`, matching the chart's `containerPort`                                          | MCP endpoint                                                                                                                                                                                                                                                                                      |
| `METRICS_LISTEN`                                               | required — `0.0.0.0:9090`                                                                                | Prometheus                                                                                                                                                                                                                                                                                        |
| `TASK_HOST` / `TASK_PORT`                                      | required — from `task.host` / `task.port`                                                                | the upstream module                                                                                                                                                                                                                                                                               |
| `IAM_HOST` / `IAM_PORT`                                        | required — from `iam.host` / `iam.port`                                                                  | the whole credential lifecycle: `/auth/login`, `/auth/enrol` and attestation                                                                                                                                                                                                                      |
| `PROJECT_HOST` / `PROJECT_PORT`                                | required — from `project.host` / `project.port`                                                          | the registry's logic tier: the set is LOADED from `ListProjects`, and the refusal path dials `ResolveProject`. Absent or unreachable does NOT fail the boot (ADR-0674)                                                                                                                            |
| `YADGAR_PROJECT_VALIDATION_MODE`                               | required — from `projectValidation.mode`, shipped as `counting`                                          | `counting` resolves, classifies and COUNTS, and refuses nobody. `enforcing` answers the refusals. No compiled-in default; an absent or misspelled value EXITS at boot (ADR-0569)                                                                                                                  |
| `YADGAR_PROJECT_REGISTRY_POLL_SECONDS`                         | required — from `projectValidation.registryPollSeconds`                                                  | how often the registered set is loaded — the retry after a failure and the refresh after a success. Absent, empty, non-numeric or `0` EXITS at boot (ADR-0569)                                                                                                                                    |
| `PROJECT_TLS_CLIENT_CERT_FILE` / `PROJECT_TLS_CLIENT_KEY_FILE` | unset                                                                                                    | the same as the two hops above, for the registry hop. Three prefixes so the three can be cut over one at a time                                                                                                                                                                                   |
...
| `YADGAR_ALLOWED_ORIGINS`                                       | required — from `cors.allowedOrigins`; EMPTY is a real answer                                            | comma-separated. Empty rejects every browser origin, which is right for a server whose clients are agents                                                                                                                                                                                         |
| `YADGAR_VALKEY_ADDR`                                           | **required**                                                                                             | the shared cache holding D74's token buckets. Unset EXITS at boot                                                                                                                                                                                                                                 |
| `YADGAR_RATE_LIMITS`                                           | required — from `rateLimit.limits`; EMPTY is a real answer                                               | `<module>.<kind>=<rate>:<burst>`, comma-separated. e.g. `task.write=2:120`                                                                                                                                                                                                                        |
| `YADGAR_RATE_LIMIT_DEFAULT`                                    | required — from `rateLimit.default`                                                                      | the bucket for a `(module, kind)` nobody named                                                                                                                                                                                                                                                    |
| `YADGAR_RATE_LIMIT_TIMEOUT_MS`                                 | required — from `rateLimit.timeoutMs`                                                                    | how long one bucket lookup may take before the call falls back to the local floor                                                                                                                                                                                                                 |
| `YADGAR_MAX_REPLICAS`                                          | required — from `autoscaling.maxReplicas`, else `replicaCount`                                           | the autoscaler's ceiling, and the divisor of the degraded floor. Unset EXITS at boot                                                                                                                                                                                                              |
...
## The workspace a caller claims, and why nothing is refused yet

Every `tools/call` and every `/admin/*` request carries `x-yadgar-project`. Until
ledger 881 the gateway checked that the header was PRESENT and nothing else: the
value went into the `Scope` unexamined, and `task-db` bound it straight into the
row. A caller could name any string on a write, and the named project's
`task_counter` advanced — a corruption of the partition key, which D52 says no
later pass can merge back.

This release resolves the claim and **refuses nobody**. The registered set is
loaded from `ProjectService.ListProjects` at boot and re-loaded on
`projectValidation.registryPollSeconds`; each claim is walked against it by
ANCESTOR, never by exact match, because a monorepo subpath declared in a
`.yadgar/project-id` marker is the DESIGNED shape (D53) and is expected to be
deeper than any registered row. The outcome is counted and the call then proceeds
exactly as it did before — including the `project_id` it is stamped with. Nothing
about the answer changes until `projectValidation.mode` says `enforcing`.

| terminal state                  | condition                                                                         | what `enforcing` answers                                   |
| ------------------------------- | --------------------------------------------------------------------------------- | ---------------------------------------------------------- |
| `MISSING_WORKSPACE`             | no `x-yadgar-project` at all — shipped in ledger 739 and unchanged here           | `400`, `INVALID_ARGUMENT`                                  |
| `PROJECT_INVALID`               | the value is not a project path; refused BEFORE resolution, and never echoed back | `400`, the grammar stated                                  |
| `PROJECT_UNREGISTERED_ORG`      | the nearest registered ancestor is a single-segment namespace ANCHOR (ADR-0673)   | `400`, "open a PR against `<source_repo>`"                 |
| `PROJECT_UNREGISTERED_PERSONAL` | a `local/` path with no registered ancestor (ADR-0605)                            | `400`, write a marker file and use the project-create tool |
| `PROJECT_UNRESOLVABLE`          | no registered ancestor at all, and not under `local/`                             | `400`, **no remediation** — ask a human                    |
| `PROJECT_REGISTRY_UNAVAILABLE`  | the registry has NEVER loaded, so no claim can be checked (ADR-0674)              | `503`, and a reason distinct from all four above           |

A namespace anchor is resolved THROUGH and is never a landing site, including when
it is named exactly: a registered `yadgarhq` exists to say "this repository is not
registered, and here is the repository whose pull-request flow governs the
namespace", not to be a bucket every unregistered repository's work pools into
(ADR-0673). Composing that remediation needs the anchor row's `source_repo`, which
the loaded set deliberately does not carry, so the refusal path — and only the
refusal path — dials `ResolveProject`. **If that dial fails the refusal still
stands**: the classification came from the loaded set, so the answer stays a `400`
carrying its reason and only the prose degrades. It never becomes a `500`.

Two series, and both are needed:

- `yadgar_gateway_project_refusal_total{reason}` — one label per state above,
  incremented in BOTH modes, so a dashboard reads continuously across the flip.
- `yadgar_gateway_project_resolution_total{outcome}` — `exact` or `ancestor`, on
  every successful resolution. Without it, a gateway serving no traffic reads
  identically to one whose canonicalisation is perfect.
- `yadgar_gateway_project_registry_loaded` — `1` once a load has EVER succeeded,
  published from boot rather than from the poll loop so "never loaded" and "never
  scraped" are different observations.

The flip is gated, and the gate is in `plans/project-validation.md`. Ledger 829
records 18 of 19 repositories unregistered, so `enforcing` before the registry is
seeded refuses essentially every call in the estate.
